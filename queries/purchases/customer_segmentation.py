...
class MostSimilarCustomers(BaseModel):
    top_k: list[GTT] = Field(description="The ordered list of top k most similar customers.")
...
@dataclass
class customer_segmentation(Test[GTT, CustomerSegmentationTestParameters]):
    name: str = "Customer Segmentation"
    name_short: str = "RFM"
    json_schema = MostSimilarCustomers
    stats_df: DataFrame = None
    named_index_col = index_name
    prompt_scoring_cols = ["StockCode", "InvoiceDate", "InvoiceNo", "TotalPrice"]
    ...
    def _parse_direct_query_manual(self, query: DirectQuery) -> bool:
    ...
    def _create_prompt_partitioned(self, df: DataFrame, target: GTT | None, q_params: PartitionedQueryParameters) -> tuple[str, str]:
        ...
        match q_params.prompt_level:
            case PromptLevel.formula:
                instruction = \
                    f"""Return the {q_params.k} customers most similar to the customer {target}. The similarity score must be computed following these steps:
    1.	Basket-Content Representation
    1.1	Build a customer–item matrix by cross-tabulating customers against the products they purchased.
    1.2	Compute the cosine similarity between customers using these vectors.
    2.	RFM Behavioral Features
...
•	{(1-self.parameters.alpha)*100:.0f}% weight for the RFM similarity"""
            case PromptLevel.instruct:
                instruction = \
                    f"""Return {q_params.k} customers most similar to customer {target} based on their purchasing behavior.
To determine similarity, evaluate customers across two standard retail dimensions, weighting them respectively {self.parameters.alpha*100:.0f}% and {(1-self.parameters.alpha)*100:.0f}%:
    ...
    2. RFM Profile (Recency, Frequency, Monetary)."""
            case PromptLevel.generic:
                instruction = f"Return the {q_params.k} customers most similar to customer {target}."
