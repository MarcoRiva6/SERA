...
from queries.test import Test, data_folder, TestParameters, PromptLevel
...
@dataclass
class global_liveability(Test[TestParameters]):
    ...
    def _load_ds(self) -> DataFrame:
        return pd.read_excel(data_folder / 'cities' / 'global_liveability.xlsx', sheet_name='Foglio2', index_col=index_col)
    ...
    def _create_prompt(self, df: DataFrame, target: str|int|None, k: int, prompt_level: PromptLevel) -> str:
        ...
        match self.run_type:
            ...
            case RunType.DIRECT:
                match prompt_level:
                    case PromptLevel.instruct:
                        instruct = f"Return the {k} most similar cities to '{target}', based only on the Global Liveability Index (GLI) using only the provided data."
                    case PromptLevel.formula:
                        instruct = f"Using only the Global Liveability Index (GLI), which can be computed with the formula {formula_string}, return the {k} most similar cities to '{target}'."
                    case PromptLevel.generic:
                        instruct = f"Return the {k} most similar cities to '{target}', based only on the provided data."
                prompt = f"{job}\n\n{instruct}\n\n{output}"
